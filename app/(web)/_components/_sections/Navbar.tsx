                ...
                <div className='hidden lg:flex items-center gap-7'>
                    <div className='flex items-center gap-4'>
                        {navLinks.map(({ title, url }) => (
                            <Link href={url} key={title} className='text-green-700 py-2 border-b-2 border-white hover:border-b-green-700 hover:border-b-2 transition-all ease-in-out duration-150'>
                                {title}
                            </Link>
                        ))}
                    </div>
